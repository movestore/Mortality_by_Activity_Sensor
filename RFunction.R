...
rFunction = function(data, min_act_change=NULL,time_itv=NULL,time_unit="hours",volt.name="tag_voltage",activity.name="activity_count",circ=256) {
 
  if (is.null(min_act_change) | is.null(time_itv))
  {
    logger.info("You have not defined the settings min_act_change or time_itv. This will lead to an error. Please go back and configure the App properly.")
  }
  ...
  } else
  {
    volt.i <- lapply(datai.ends, function(x)
    {
      ...
      if (dim(x)[1]>=2)
      {
        ...
        data.frame("individual_local_identifier"=mt_track_id(x)[1],"number_of_measuremens"=len,"voltage_end"=voltage.end,"est_voltage_change_rate"=lin.slp)
      } else if (dim(x)[1]==1) #special case 1 measurement, no change rate possible
      {
        ...
        eval(parse(text=paste0("voltage <- x$",volt.name))) 
        voltage.end <- voltage
        data.frame("individual_local_identifier"=mt_track_id(x)[1],"number_of_measuremens"=1,"voltage_end"=voltage.end,"est_voltage_change_rate"=NA)
      } else #should never reach this branch
      {
        ...
        data.frame("individual_local_identifier"=character(),"number_of_measuremens"=numeric(),"voltage_end"=numeric(),"est_voltage_change_rate"=numeric())
      }
    })
  ...
  } else
  {
    ch.act.i <- lapply(datai.ends, function(x)
    {
      if (dim(x)[1]>=2)
      {
        ...
        ts <- mt_time(x)[max(ixs)] #later timestamp of maxdiff measurement
        alert <- 0
      ...
      {
        logger.info(paste("For the track",mt_track_id(x)[1],"only one measurement lies in the defined time interval. Therefore, no activity difference can be calculated and no alert is set."))
        ts <- mt_time(x)
  ...
  if (exists("alert_file")) 
  {
    ...
    result <- mt_set_track_data(data,left_join(mt_track_data(data),alert_file))
  } else result <- data
  
  return(result)
}

#return only end time interval of tracks with alert